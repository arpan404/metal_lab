...
export class AtomicDeflection extends BaseGame {
  private experiment: RutherfordGoldFoil;
  private targetZones: TargetZone[];
  private currentChallenge: DeflectionChallenge | null = null;
  private challengeStartTime: number = 0;
  private particlesLaunched: number = 0;
  private particlesHit: number = 0;
  ...
  constructor(config: GameConfig) {
    ...
    this.targetZones = (config as any).targetZones || [
   ...
   */
  private getPointsForAngle(angle: number): number {
   ...
   */
  private checkParticleHits(): void {
    const particles = this.experiment.getActiveParticles();
    
    particles.forEach(particle => {
      if (!particle.detected) return;
   ...
   * Hit target
   */
  private hitTarget(angle: number, error: number): void {
