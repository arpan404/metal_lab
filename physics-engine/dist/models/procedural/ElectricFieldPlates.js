...
export class ElectricFieldPlates {
    ...
    setVoltage(voltage) {
        ...
        if (this.upperPlate) {
            const material = this.upperPlate.material;
            const intensity = Math.min(Math.abs(voltage) / 1000, 0.5);
            material.emissiveIntensity = intensity;
        }
        if (this.lowerPlate) {
            const material = this.lowerPlate.material;
            const intensity = Math.min(Math.abs(voltage) / 1000, 0.5);
            material.emissiveIntensity = intensity;
        }
    }
    ...
    dispose() {
        ...
        (this.upperPlate?.material).dispose();
        this.lowerPlate?.geometry.dispose();
        (this.lowerPlate?.material).dispose();
