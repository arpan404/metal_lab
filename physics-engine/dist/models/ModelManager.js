...
import { BankedTrack, ElectricFieldPlates, GoldFoilSetup, DoubleSlitBarrier, PendulumRig, MillikanChamber, ParticleEmitter, MeasurementGrid, } from './procedural';
export class ModelManager {
    constructor(assetPath = '/assets/models/') {
        this.models = new Map();
     ...
     */
    createProcedural(modelType, config) {
        let object;
        switch (modelType) {
            case 'banked-track':
    ...
    async preloadCommon() {
        const promises = [];
        // Load GLTF models
        promises.push(this.loadGLTF('car').catch(err => console.warn('Car model not found:', err)), this.loadGLTF('lab-stand').catch(err => console.warn('Lab stand model not found:', err)));
        await Promise.all(promises);
    }
    /**
     * Apply standard material properties to loaded models
     */
    applyStandardMaterial(modelType, properties) {
