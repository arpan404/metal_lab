...
export class ShaderLoader {
    constructor(preferredBackend = 'webgpu') {
     ...
     */
    async loadShader(name, type = 'wgsl') {
     ...
     */
    async loadShaderForBackend(name) {
     ...
     */
    extractEntryPoints(source, type) {
        const entryPoints = [];
        if (type === 'wgsl') {
    ...
    async isMetalBackend() {
        ...
        try {
            ...
            return (info.architecture?.toLowerCase().includes('apple') ||
     ...
     */
    async preloadShaders(names, type = 'wgsl') {
 ...
 */
export async function loadShader(name, type = 'wgsl') {
    return shaderLoader.loadShader(name, type);
}
/**
 * Utility function to load shader for current backend
 */
export async function loadShaderForBackend(name) {
