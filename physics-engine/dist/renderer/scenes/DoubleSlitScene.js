...
export class DoubleSlitScene extends BaseScene {
    ...
    createWavePlane() {
        ...
        this.waveTexture = new THREE.DataTexture(bytes, gridSize, gridSize, THREE.RGBAFormat, THREE.UnsignedByteType);
    ...
    update(_deltaTime) {
        ...
        if (this.waveTexture && waveField) {
            ...
            const raw = img instanceof ImageData
                ? img.data
                : img.data;
            const data = raw instanceof Uint8Array
                ? raw
                : new Uint8Array(raw.buffer, raw.byteOffset ?? 0, raw.byteLength ?? raw.byteLength);
    ...
    updateInterferencePattern() {
        if (!this.patternCanvas || !this.screen)
            return;
