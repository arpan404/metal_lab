...
export class WaveVisualization {
    constructor(scene) {
        this.wavePlanes = new Map();
        this.scene = scene;
    }
    /**
     * Create wave visualization plane
     */
    createWavePlane(id, config = {}) {
     ...
     */
    updateWavePlane(id, waveData, config = {}) {
     ...
     */
    update3DWaveSurface(id, heightData, scale = 1.0) {
        const wavePlane = this.wavePlanes.get(id);
        if (!wavePlane || !wavePlane.mesh.geometry)
            return;
     ...
     */
    createInterferencePattern(id, source1, source2, wavelength, config = {}) {
        ...
        for (let y = 0; y < resolution; y++) {
            for (let x = 0; x < resolution; x++) {
                ...
                const d1 = Math.sqrt(Math.pow(px - source1.x, 2) + Math.pow(py - source1.y, 2));
                const d2 = Math.sqrt(Math.pow(px - source2.x, 2) + Math.pow(py - source2.y, 2));
     ...
     */
    animateWave(id, time, frequency = 1.0) {
        const wavePlane = this.wavePlanes.get(id);
        if (!wavePlane)
            return;
        const resolution = wavePlane.resolution;
