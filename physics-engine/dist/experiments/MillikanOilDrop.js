...
export class MillikanOilDrop extends BaseExperiment {
    constructor() {
        ...
        super('Millikan Oil Drop', 'Measure the elementary charge by balancing charged oil droplets', parameters, objectives);
        this.droplets = new Map();
        this.nextDropletId = 0;
        this.plateVoltage = 5000; // volts
        this.plateSeparation = 0.01; // meters
    ...
    measureDropletCharge(dropletId) {
    ...
    calculateQuantizationConfidence() {
        if (this.measuredCharges.length < 3)
            return 0;
    ...
    getMeasurements() {
        const firstDroplet = this.droplets.values().next().value;
        return {
            elapsedTime: this.elapsedTime,
