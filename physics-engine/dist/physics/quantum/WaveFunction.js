...
export class WaveFunction {
     ...
     */
    static createGaussianWavePacket(centerX, centerY, sigmaX, sigmaY, k0X, k0Y, gridSize) {
        ...
        for (let y = 0; y < gridSize; y++) {
            for (let x = 0; x < gridSize; x++) {
                ...
                const dy = y - centerY;
                // Gaussian envelope
                const envelope = Math.exp(-(dx * dx) / (2 * sigmaX * sigmaX) -
     ...
     * Create plane wave
     */
    static createPlaneWave(kX, kY, gridSize) {
        const real = new Float32Array(gridSize * gridSize);
        const imaginary = new Float32Array(gridSize * gridSize);
        const probability = new Float32Array(gridSize * gridSize);
        for (let y = 0; y < gridSize; y++) {
     ...
     */
    static calculateProbability(real, imaginary) {
     ...
     */
    static expectationPosition(state, gridSize) {
        let sumX = 0;
        let sumY = 0;
        for (let y = 0; y < gridSize; y++) {
            for (let x = 0; x < gridSize; x++) {
     ...
     */
    static expectationMomentum(state, gridSize, dx) {
        let sumPx = 0;
        let sumPy = 0;
     ...
     */
    static uncertaintyPosition(state, gridSize) {
     ...
     * E = ℏω = ℏck
     */
    static energyFromWaveNumber(k, c = 299792458) {
        const hbar = 1.054571817e-34;
        return hbar * c * k;
    }
}
