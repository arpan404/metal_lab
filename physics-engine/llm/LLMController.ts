// physics-engine/llm/LLMController.ts
...
export class LLMController {
  ...
  private isPausedByLLM: boolean = false;
  private currentExplanation: LLMExplanation | null = null;
  private explanationQueue: LLMExplanation[] = [];
  private activeExplanationPoints: Set<string> = new Set();
  
  // Callbacks for LLM events
  private onExplanationCallback?: (explanation: LLMExplanation) => void;
  ...
  checkExplanationPoints(experiment: BaseExperiment): void {
    ...
    for (const point of points) {
      ...
      if (this.activeExplanationPoints.has(point.id)) continue;
