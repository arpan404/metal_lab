...
export class NASCARScene extends BaseScene {
  ...
  private createForceVectors(): void {
    ...
    // Friction force (blue)
    const friction = new THREE.ArrowHelper(
      new THREE.Vector3(0, 0, 0),
      new THREE.Vector3(0, 0, 0),
      arrowLength,
      0x0000ff
  ...
  showForceVectors(show: boolean): void {
    ...
    if (centripetal) centripetal.visible = show;
    if (normal) normal.visible = show;
    if (friction) friction.visible = show;
  }
