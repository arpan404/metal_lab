...
export class MeasurementTools {
  ...
  private tools: Map<string, THREE.Object3D> = new Map();
  ...
  addProtractor(
    ...
    for (let i = 0; i <= divisions; i++) {
      ...
      const markEnd = new THREE.Vector3(
        center.x + (radius * 1.1) * Math.cos(angle),
        center.y + (radius * 1.1) * Math.sin(angle),
        center.z
      );
   ...
   * Add coordinate axes
   */
  addAxes(id: string, size: number = 10): void {
  ...
  updateVectorArrow(
    ...
    const arrow = this.tools.get(id) as THREE.ArrowHelper;
    if (arrow) {
   ...
   * Add bounding box
   */
  addBoundingBox(id: string, object: THREE.Object3D, color: number = 0xffff00): void {
