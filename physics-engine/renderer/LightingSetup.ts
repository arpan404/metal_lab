...
export class LightingSetup {
   ...
   */
  static createCustomLighting(
    scene: THREE.Scene,
    config: {
      ambient?: { color: number; intensity: number };
      directional?: Array<{ color: number; intensity: number; position: number[] }>;
      point?: Array<{ color: number; intensity: number; position: number[]; distance?: number }>;
      spot?: Array<{ color: number; intensity: number; position: number[]; target?: number[] }>;
    }
  ): void {
    // Ambient
    if (config.ambient) {
