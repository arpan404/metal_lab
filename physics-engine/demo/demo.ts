...
import { WaveVisualization } from '../renderer/effects/WaveVisualization';
import type { BaseExperiment } from '../experiments/BaseExperiment';
import type { BaseScene } from '../renderer/scenes/BaseScene';

interface ExperimentConfig {
  name: string;
  description: string;
  experimentClass: new () => BaseExperiment;
  sceneClass: new (scene: any, experiment: any) => BaseScene;
  controls: Array<{
    name: string;
    label: string;
    min: number;
...
class VirtualPhysicsLab {
  ...
  private currentExperimentName: string = 'wave-field';
  ...
  private experiments: Record<string, ExperimentConfig> = {
    'wave-field': {
      name: 'Wave Field Visualization',
      description: 'Real quantum wave interference using wave equation solver',
      experimentClass: YoungDoubleSlit as any, // Will use for wave visualization
      sceneClass: DoubleSlitScene as any,
      controls: [
  ...
  setupUI() {
    ...
    document.querySelectorAll('.demo-item').forEach(item => {
      item.addEventListener('click', () => {
        const demoName = (item as HTMLElement).dataset.demo;
  ...
  async loadExperiment(name: string) {
    ...
    try {
      ...
      if (titleEl) titleEl.textContent = expConfig.name;
      if (descEl) descEl.textContent = expConfig.description;
  ...
  setupControls(controls: ExperimentConfig['controls']) {
  ...
  updateMeasurements() {
    ...
    Object.entries(measurements).forEach(([name, value]) => {
      ...
      if (name === 'elapsedTime' || name === 'latitude_changes') return;
...
window.addEventListener('DOMContentLoaded', () => {
  (window as any).lab = new VirtualPhysicsLab();
});
