// physics-engine/physics/quantum/SchrodingerSolver.ts
...
export class SchrodingerSolver {
  private static readonly hbar = 1.054571817e-34;
  ...
  private static applyKineticOperator(
    ...
    for (let y = 1; y < gridSize - 1; y++) {
      for (let x = 1; x < gridSize - 1; x++) {
        ...
        const laplacianReal = 
          state.real[idx - 1] + state.real[idx + 1] +
          state.real[idx - gridSize] + state.real[idx + gridSize] -
          4 * state.real[idx];
        
        const laplacianImag = 
          state.imaginary[idx - 1] + state.imaginary[idx + 1] +
