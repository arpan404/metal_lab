// physics-engine/physics/nuclear/CoulombForce.ts
...
export class CoulombForce {
  ...
  private static readonly e = 1.602176634e-19; // Elementary charge (C)
  
  /**
   * Calculate Coulomb force magnitude
   * F = k·q₁·q₂/r²
   */
  static forceMagnitude(
    charge1: number,
    charge2: number,
    distance: number
  ): number {
    if (distance < 1e-15) return Infinity;
  ...
  static forceVector(
    ...
    if (distance < 1e-15) return { x: 0, y: 0, z: 0 };
   ...
   * a₀ = ℏ²/(m·k·e²)
   */
  static bohrRadius(mass: number, charge: number = 1): number {
