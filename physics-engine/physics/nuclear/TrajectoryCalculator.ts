// physics-engine/physics/nuclear/TrajectoryCalculator.ts
...
export class TrajectoryCalculator {
  ...
  static calculateTrajectory(
    ...
    nucleusPosition: Vector3D = { x: 0, y: 0, z: 0 },
    duration: number,
    steps: number = 1000
  ): Vector3D[] {
  ...
  private static acceleration(
    position: Vector3D,
    nucleusPosition: Vector3D,
    params: ScatteringParams
  ): Vector3D {
    const force = CoulombForce.forceVector(
      params.charge1,
      position,
      params.charge2,
      nucleusPosition
   ...
   */
  static findClosestApproach(
    trajectory: Vector3D[],
    nucleusPosition: Vector3D = { x: 0, y: 0, z: 0 }
  ): { distance: number; position: Vector3D; index: number } {
   ...
   */
  static calculateScatteringAngle(
    trajectory: Vector3D[],
    ...
    // Normalize
    const magInitial = Math.sqrt(
      initialVelocity.x ** 2 + initialVelocity.y ** 2 + initialVelocity.z ** 2
    );
    const magFinal = Math.sqrt(
      finalDirection.x ** 2 + finalDirection.y ** 2 + finalDirection.z ** 2
    );
    
    if (magInitial === 0 || magFinal === 0) return 0;
    
    // Dot product
    const dot = 
      (initialVelocity.x * finalDirection.x +
  ...
  static hitsDetector(
    ...
    detectorAngleMin: number,
    detectorAngleMax: number,
    nucleusPosition: Vector3D = { x: 0, y: 0, z: 0 }
  ): boolean {
