// physics-engine/physics/nuclear/RutherfordScattering.ts
import type { ScatteringParams, ScatteringResult, Vector3D } from '../../types';
...
export class RutherfordScattering {
   ...
   */
  static differentialCrossSection(
    energy: number,
    charge1: number,
    charge2: number,
    scatteringAngle: number
  ...
  static calculateTrajectory(
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      if (r < 1e-15) break; // Avoid singularity
      ...
      // Stop if far enough
      if (position.x > 5 * closest && r > 10 * closest) break;
    }
  ...
  static estimateNuclearRadius(
    ...
    return this.closestApproach(
      { impactParameter: 0, energy, charge1, charge2, mass: 6.64e-27 },
      minObservedAngle
    );
  }
