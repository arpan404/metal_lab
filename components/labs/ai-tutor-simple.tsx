...
import { ArrowRightIcon, BookOpenIcon, XIcon, SparklesIcon, ArrowLeftIcon, RotateCcwIcon } from "lucide-react";
...
- Make it engaging and educational`;
    ...
    return {
      ...
      detailedExplanation: generatedContent.detailedExplanation || "Content being generated...",
      keyPoints: Array.isArray(generatedContent.keyPoints) ? generatedContent.keyPoints : [],
...
const getResearchLinks = (stepTitle: string): ResearchLink[] => {
  const linkMap: Record<string, ResearchLink[]> = {
    "Input Tokens": [
      { title: "Tokenization in NLP", url: "https://huggingface.co/course/chapter2/4", description: "Comprehensive guide to tokenization methods" },
      { title: "BPE Tokenization Paper", url: "https://arxiv.org/abs/1508.07909", description: "Neural Machine Translation of Rare Words" },
      { title: "SentencePiece", url: "https://github.com/google/sentencepiece", description: "Unsupervised text tokenizer" },
    ],
    "Embeddings": [
      { title: "Word2Vec Paper", url: "https://arxiv.org/abs/1301.3781", description: "Efficient Estimation of Word Representations" },
      { title: "Understanding Embeddings", url: "https://jalammar.github.io/illustrated-word2vec/", description: "Visual guide to word embeddings" },
      { title: "Positional Encoding", url: "https://kazemnejad.com/blog/transformer_architecture_positional_encoding/", description: "Deep dive into positional encodings" },
    ],
    "QKV Projection": [
      { title: "Attention Is All You Need", url: "https://arxiv.org/abs/1706.03762", description: "The original Transformer paper" },
      { title: "Illustrated Self-Attention", url: "https://jalammar.github.io/illustrated-transformer/", description: "Visual explanation of attention" },
      { title: "QKV Intuition", url: "https://www.youtube.com/watch?v=KmAISyVvE1Y", description: "Stanford CS224N: Attention mechanisms" },
    ],
    "Attention Mechanism": [
      { title: "Attention Is All You Need", url: "https://arxiv.org/abs/1706.03762", description: "Original paper on multi-head attention" },
      { title: "Multi-Head Attention Explained", url: "https://jalammar.github.io/illustrated-transformer/", description: "Visual guide" },
      { title: "Attention Visualization", url: "https://arxiv.org/abs/1906.04341", description: "What Does BERT Look At?" },
    ],
    "Feed-Forward Network": [
      { title: "FFN in Transformers", url: "https://arxiv.org/abs/1706.03762", description: "Section 3.3 of original paper" },
      { title: "GLU Variants", url: "https://arxiv.org/abs/2002.05202", description: "Improved FFN architectures" },
      { title: "Understanding FFN", url: "https://www.youtube.com/watch?v=kCc8FmEb1nY", description: "Andrej Karpathy's explanation" },
    ],
    "Layer Normalization": [
      { title: "Layer Norm Paper", url: "https://arxiv.org/abs/1607.06450", description: "Original Layer Normalization paper" },
      { title: "RMSNorm", url: "https://arxiv.org/abs/1910.07467", description: "Root Mean Square Layer Normalization" },
      { title: "Norm Techniques Comparison", url: "https://arxiv.org/abs/2009.10739", description: "Understanding normalization" },
    ],
    "Output Hidden States": [
      { title: "Language Modeling Heads", url: "https://arxiv.org/abs/1706.03762", description: "Transformer architecture" },
      { title: "Tied Embeddings", url: "https://arxiv.org/abs/1608.05859", description: "Using the same weight matrix" },
      { title: "GPT Architecture", url: "https://cdn.openai.com/research-covers/language-unsupervised/language_understanding_paper.pdf", description: "GPT model details" },
    ],
    "Softmax & Prediction": [
      { title: "Softmax Function", url: "https://en.wikipedia.org/wiki/Softmax_function", description: "Mathematical foundation" },
      { title: "Temperature in Sampling", url: "https://arxiv.org/abs/1904.09751", description: "The Curious Case of Neural Text Degeneration" },
      { title: "Decoding Strategies", url: "https://huggingface.co/blog/how-to-generate", description: "Comprehensive guide to text generation" },
    ],
  };

  // Return links for the specific step or default to general transformer resources
  return linkMap[stepTitle] || [
    { title: "Attention Is All You Need", url: "https://arxiv.org/abs/1706.03762", description: "Original Transformer paper" },
    { title: "The Illustrated Transformer", url: "https://jalammar.github.io/illustrated-transformer/", description: "Visual guide" },
    { title: "Andrej Karpathy: Let's Build GPT", url: "https://www.youtube.com/watch?v=kCc8FmEb1nY", description: "Video tutorial" },
  ];
};
...
const stepContentData: StepContent[] = [
  {
    ...
    quiz: [
      ...
      {
        question: "Why do we use subword tokenization instead of just word-level?",
        options: [
      ...
      {
        question: "In the phrase 'unbelievable', how might BPE tokenization split it?",
        options: [
...
export function SimpleAITutor() {
  ...
  const [showContinuousGenerationModal, setShowContinuousGenerationModal] = useState(false);
  ...
  useEffect(() => {
    if (hasStarted && !isPreloading) {
      ...
      preloadAllContent().then(() => {
        console.log("✅ Content preloading complete - all hover details cached");
    ...
    try {
      const response = await fetch("/api/ai/simulate-transformer", {
        ...
        body: JSON.stringify({
          prompts: [{ 
            role: "user", 
            content: `Suggest a simple 1 word phrase (like "Hello" or "Cat" or "Dog") that would be good for demonstrating how a transformer processes text. Just respond with the phrase only, nothing else. Current input is: "${inputText}"` 
          }],
          tools: [],
        }),
      });
      ...
      const data = await response.json();
      const suggested = data.content?.trim() || "Hello";
      setSuggestedInput(suggested);
      return suggested;
    } catch (error) {
      console.error("Error getting suggestion:", error);
      const fallback = "Hello";
      setSuggestedInput(fallback);
      return fallback;
    }
  };
  ...
  const handleContinue = () => {
    if (currentStep < 7) {
      ...
      if (!hasCompletedOnce) {
        ...
        setShowContinuousGenerationModal(true);
      } else {
        setExplanation("Great work! You've completed the tour. Enable Auto-Continue to see continuous generation!");
      }
    }
  };
      ...
      <>
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            {hasCompletedOnce && (
              <Button
                className="cursor-pointer w-full bg-linear-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white font-semibold shadow-lg"
                variant="default"
                size="sm"
                onClick={() => {
                  setAutoContinue(true);
                  setExplanation("Auto-Generate enabled! The model will now continuously generate tokens. Watch the magic! ✨");
                }}
        ...
        <ContinuousGenerationModal
          ...
          onEnableAutoContinue={() => {
            ...
            setExplanation("Auto-Continue enabled! Watch how LLMs generate tokens continuously!");
          }}
        />
      </>
    );
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-gray-700 leading-relaxed mb-4">
              LLMs generate text by looping through all 8 steps repeatedly. Each predicted token becomes the new input!
            </p>
            ...
            <div className="bg-linear-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4 mb-4">
              ...
              <div className="flex items-center justify-between text-sm text-gray-700 font-medium">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-blue-900">Predict</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-purple-900">Append</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-pink-900">Repeat</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-linear-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4">
              <p className="text-gray-700 text-sm leading-relaxed">
                <strong className="text-green-900">💡 This is how ChatGPT works!</strong> Each token cycles through all 8 steps until completion.
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm mb-4">
              Enable <strong>Auto-Continue</strong> to watch the continuous generation loop!
            </p>
          </div>
...
function DetailedContentModal({
  ...
  const isContentLoading = content.detailedExplanation.includes("being generated");
                  ...
                  >
                    <div className="flex items-start gap-2 text-blue-600 hover:text-blue-800 transition-colors">
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-sm group-hover:underline">{link.title}</div>
...
function QuizComponent({
  ...
  const generateQuizQuestions = async (count: number = 5): Promise<QuizQuestion[]> => {
    try {
      ...
      <div className="space-y-8 text-center py-12">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-600 max-w-md mx-auto">
            Mela is analyzing the content and generating personalized questions to test your understanding
          </p>
        </div>
        ...
        <div className="bg-linear-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl p-4 max-w-md mx-auto">
          <p className="text-xs text-gray-700">
            💡 <strong>Pro tip:</strong> These questions are AI-generated based on the specific content you learned
          </p>
        </div>
      </div>
    );
